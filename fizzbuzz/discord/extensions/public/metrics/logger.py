"""Log metrics for the wrapped event."""
...
logger = getLogger("WrappedLoggerCog")


class WrappedLoggerCog(commands.Cog):
    """Logs messages for wrapped and backfills missed ones on startup."""
    ...
    async def _run_catchup(self) -> None:
        ...
        if current_year < FIRST_YEAR:
            logger.info("Wrapped catchup: current year is before FIRST_YEAR, skipping.")
            return
        ...
        logger.info(
            "Wrapped catchup: year=%s, window %s -> %s",
            current_year,
        ...
        if guild is None:
            logger.error(
                "FizzBuzz is not in the cat server! "
        ...
        for channel in text_channels:
            try:
                ...
                logger.exception(
                    "Wrapped: failed on channel #%s (%s)", channel.name, channel.id
                )

        logger.info("Wrapped catchup: finished.")
        ...
        *,
        ...
        async for message in channel.history(
            ...
            try:
                ...
                logger.exception(
                    "Wrapped catchup: failed logging message %s in #%s",
                    message.id,
                    channel.name,
                )
